...
List bayesian_hclustcc_restart_cpp(const List nb,const List X,NumericMatrix starting_merge,List method_obj,bool display_progress,bool approx) {
  ...
  for(int imerge=0;imerge<(V-1);imerge++){
    ...
    if(imerge >= starting_merge.nrow()){
      ...
      g = starting_merge(imerge,0);
      h = starting_merge(imerge,1);
      // store curent height
      bayesian_node node_g = graph[g];
      // trouver h dans node_g.neibs
      auto search = node_g.neibs.find(h);
      if (search != node_g.neibs.end()){
        multiedge uv = search->second;
        // recupérer cheight la hauteur
        double cheight = uv.height;
        height[imerge]=cheight;
        // link deletion in priority_queue
        // pas besoin ce merge sera supprimer de la priority queue au moment de la suppression des voisins de g et de h
        // supprimer g-h de la priority_queue en cherchant avec cheight
        //auto search = priority_queue.equal_range(cheight);
        //for (auto s = search.first; s != search.second; ++s){
        //  std::pair<int,int> edge = s->second;
        //  if(std::get<0>(edge)==std::min(g,h) && std::get<1>(edge)==std::max(g,h)){
        //    priority_queue.erase(s);
        //    break;
        //  }
        //}
