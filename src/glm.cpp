...
double glm_laplace_evidence(const arma::mat& X, const arma::colvec& y, List fit){
  ...
  const arma::colvec beta  = as<arma::colvec>(fit["beta"]); 
  const arma::mat S  = as<arma::mat>(fit["iH"]); 
  int D = beta.n_rows;
  const double lambda =  as<double>(fit["lambda"]); 
  return ll+0.5*D*log(2*M_PI)+0.5*arma::log_det_sympd(S);
}
...
double delta_merge_post(List fit1,List fit2,double lambda){
  ...
  const arma::mat S1 = as<arma::mat>(fit1["iH"]);
  const arma::mat S2 = as<arma::mat>(fit2["iH"]);

  const arma::mat H12 = H1+H2;
  const arma::mat S12 = arma::inv_sympd(H1+H2);
  const arma::colvec beta12 = S12*(H1*beta1+H2*beta2);
  Rcout << beta12 << std::endl;
  double qdets = -arma::log_det_sympd(iLambdas-S12)-D*log(lambda);
  double delta = qdets + log_mvn_pdf(beta1,beta2,S1+S2)-log_mvn_pdf(arma::colvec(D, arma::fill::zeros),beta12,iLambdas-S12);
  return delta;
}
